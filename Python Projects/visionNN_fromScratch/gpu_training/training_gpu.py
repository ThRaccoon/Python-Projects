# ==================================================
#  Neural Network: CuPy Implementation (v12.4)
# ==================================================
# This implementation uses CuPy (v12.4) to leverage GPU
# acceleration for faster computations. CuPy’s ndarray
# mimics NumPy’s interface but performs operations on
# the GPU, providing significant speedups for large-scale
# matrix operations and deep learning tasks.
# ==================================================
...
class InitParams:
    ...
    def create_inputs(n_batches: int, n_inputs: int):
        ...
        output_matrix = cp.random.random((n_batches, n_inputs))
    ...
    def create_weights(n_inputs: int, n_neurons: int):
        ...
        output_matrix = cp.random.normal(0, std_dev, (n_inputs, n_neurons))

        return output_matrix

    @staticmethod
    def create_biases(weight_matrix: cp.ndarray):

        output_vector = cp.zeros(weight_matrix.shape[1])
...
class LayerDense:
    ...
    def __init__(self, n_inputs: int, n_neurons: int, load_from_file=False):
        ...
        # Backward
        self.g_weights = cp.empty((0, 0))
        self.g_biases = cp.empty((0, 0))
    ...
    def backward(self, relu_output: cp.ndarray, g_activation_output: cp.ndarray):

        # Calculating gradients for weights and biases
        self.g_weights = cp.dot(relu_output.T, g_activation_output)
        self.g_biases = cp.sum(g_activation_output, axis=0)
        ...
        # Weights
        sqrt_sum = cp.sum(self.g_weights ** 2)
        ...
        # Biases
        sqrt_sum = cp.sum(self.g_biases ** 2)
    ...
    def read_weights_and_biases_from_file(self):
        with cp.load("files/weights&biases_cp.npz") as _file:
...
class ActivationReLU:
    def __init__(self):
        ...
        # Backward
        self.g_relu_input = cp.empty((0, 0))
        self.g_relu_output = cp.empty((0, 0))
    ...
    def backward(self, g_activation_output: cp.ndarray,
                 weights: cp.ndarray):

        # Calculating gradients for relu input (since it's backward it's from right to left)
        self.g_relu_input = cp.dot(g_activation_output, weights.T)

        # Calculating gradients for relu output
        d_relu = cp.where(self.input_matrix > 0, 1, 0)
...
class ActivationSoftmax:
    def __init__(self):
        ...
        # Backward
        self.g_softmax_output = cp.empty((0, 0))
    ...
    def backward(self, y_true: cp.ndarray):
        ...
        self.g_softmax_output[cp.arange(len(y_true)), y_true] -= 1
...
def update_weights_and_biases_file():
    cp.savez("files/weights&biases_cp.npz",
             l1_w=layer1.weights,
             l1_b=layer1.biases,
             l2_w=layer2.weights,
             l2_b=layer2.biases,
