...
new_screen_weight = 800  # Change but keep aspect ratio 1:1 or 4:3
new_screen_height = 800  # Change but keep aspect ratio 1:1 or 4:3
DEFAULT_COLLIDER = 33
DEFAULT_OFFSET = 18
...
score = Score(-100, new_screen_height / 2 - 50)

with open("data.txt", "r") as file:
    score.record = file.read()

score.show_score()
game_over_message = Score((new_screen_weight / 4) * -1, 0)
...
game_on = True
while game_on:
    player.player_boundaries(player.player.xcor(), player.player.ycor())
    list_of_cars += spawn_cars()
    ...
    for i in range(len(list_of_cars)):
        if player.player.distance(list_of_cars[i].car_x_cor(), list_of_cars[i].car_y_cor()) <= collider:
            score.record = int(score.record)
            score.update_record()
            with open("data.txt", "w") as file:
                file.write(repr(score.record))
            game_over_message.write("You got hit by a car!", False, "left", ("Verdana", 15, "normal"))
            player.player.hideturtle()
            for k in range(len(list_of_cars) - 1, -1, -1):
                list_of_cars[k].car.hideturtle()
            game_on = False
